...
n = 512000 #8192  # Number of grains
z_top = 32.0 # for 16000, z_top = 1.0
...
dt = utils.pwave_timestep(grain_r_min, youngs_mod, density, .5) # Larger dt might lead to unstable results.
print(dt)
...
@ti.kernel
def apply_bc():
    bounce_coef = 0.  # Velocity damping
    for i in gf:
        x = gf[i].p[0]
        y = gf[i].p[1]
